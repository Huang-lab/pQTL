...
for (cancer in pan.coh) {
  ...
  for (mut in mut.types) {
    ...
    # inst = 'JHU' # for OV
    # df = read_excel(paste0('../data/Pro_regression_retro_cptac/Table.DNA.PRO.', inst,'.regression.linearLIMMA.ProVsMut.', cancer, '.', mut, '.xlsx'))
    df$mutation = mut
    dat = rbind(dat, df)
  }
    ...
    # ggsave(paste0('../doc/retro_volcano/OV', inst, 'pQTL.png'), w=2.5,h=2.5) 
    print(p)
    dev.off()
}
